...
public class SiteParameterWindow extends JDialog {
	...
	public SiteParameterWindow(ExParameter templateparam, GUI gu, JFileChooser f) {
		...
		final JComboBox cmbChrom = new JComboBox();
				...
				try {
					if(param.getInput().size() < 1) {
						...
						for(int i = 0; i < param.getInput().size(); i++){
							XmlFile file = new XmlFile("Single-Site-Extraction", param.getInput().get(i).getPath(), cmbChrom.getSelectedItem().toString(), Integer.parseInt(txtCenter.getText()), ChromosomeSize, param.getStrand(), Integer.parseInt(txtWind.getText()), Integer.parseInt(txtThread.getText()), Integer.parseInt(txtRes.getText()), Integer.parseInt(txtExt.getText()), param.getStrand(), Integer.parseInt(txtShift.getText()), param.getTrans(),Integer.parseInt(txtStdDevSize.getText()), Integer.parseInt(txtNumStdDev.getText()), Integer.parseInt(txtCondWindow.getText()), Integer.parseInt(txtTagCount.getText()));
							file.createSingleSiteExtractXML();
						}
					}
